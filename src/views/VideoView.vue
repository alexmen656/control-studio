...
const currentVideo = { title: router.params.id }
const API_URL = 'http://localhost:6709/api'

/// api / videos /: id
axios.get(API_URL + '/videos/' + router.params.id).then((result) => {
    console.log(result);
});
    ...
    <div class="h-full flex flex-col bg-white dark:bg-gray-900">
        <div class="px-8 py-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Video: {{ currentVideo.title }}</h1>
            ...
            <div class="flex flex-wrap items-center gap-4">
                <div class="flex-1 min-w-[200px] max-w-md">
                    <div class="relative">
                        <svg class="absolute left-3 top-2.5 w-5 h-5 text-gray-400" fill="currentColor"
                            viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
                        ...
                        <tr v-for="video in filteredVideos" :key="video.id"
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="relative w-20 h-12 bg-gray-200 dark:bg-gray-700 rounded overflow-hidden flex-shrink-0">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <div v-if="video.progress !== undefined" class="mt-1">
                                            <div
                                                class="w-32 bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                            ...
                            <td class="px-6 py-4">
                                <span
                                    :class="['text-xs px-2 py-1 rounded-full font-medium', getStatusColor(video.status)]">
                                    {{ video.status.charAt(0).toUpperCase() + video.status.slice(1) }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-1">
                                    <span v-for="platform in video.platforms" :key="platform" class="text-lg"
                                        :title="platform">
                                        {{ getPlatformIcon(platform) }}
                                    </span>
                                </div>
